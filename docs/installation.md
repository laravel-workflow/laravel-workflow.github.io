...
> ✨ SQS Support: `WorkflowStub::timer()` and `WorkflowStub::awaitWithTimeout()` work with any duration, even when using the SQS queue driver. Laravel Workflow automatically handles SQS's delay limitation transparently.
